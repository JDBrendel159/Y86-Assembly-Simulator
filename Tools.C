...
{
   if (byteNum >= LONGSIZE || byteNum < 0)
       return 0;

   int x;
   for (x = byteNum; x > 0; x--)
       source = source >> LONGSIZE;
...
{
   if (byteNum < 0 || byteNum > (LONGSIZE - 1))
       return source;

   uint64_t temp = 0xffffffffffffffff;
   temp = getByte(temp, byteNum);
   temp <<= (byteNum * LONGSIZE);
   return source | temp;
}
...
bool Tools::addOverflow(uint64_t op1, uint64_t op2)
{
   ...
   int64_t newOp1 = 0;
   int64_t newOp2 = 0;
   newOp1 = copyBits(op1, newOp1, 0, 0, 64);
   newOp2 = copyBits(op2, newOp2, 0, 0, 64);
   if (sign((newOp1 + newOp2)) != sign(newOp1))
       return true;
   else
       return false;
}
...
bool Tools::subOverflow(uint64_t op1, uint64_t op2)
{
   ...
   newOp1 = copyBits(op1, newOp1, 0, 0, 64);
   newOp2 = copyBits(op2, newOp2, 0, 0, 64);
   if (sign(newOp1) == sign (newOp2) || sign(newOp2 - newOp1) == sign(newOp2))
       return false;
   else
       return true;
}
